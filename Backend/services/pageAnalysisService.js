...
exports.scrapeDetailedPageData = async (url) => {
  ...
  page.on("response", async (response) => {
    try {
      ...
      console.warn(
        "Error processing response in pageAnalysisService:",
        e.message
      );
    }
  });
  ...
  try {
    ...
    const domData = await page.evaluate(() => {
      return {
        ...
        externalScripts: Array.from(document.scripts).filter((s) => s.src)
          .length,
        inlineScripts: Array.from(document.scripts).filter((s) => !s.src)
          .length,
        stylesheets: Array.from(document.styleSheets).length,
        inlineStyles: document.querySelectorAll("style").length,
        imagesWithoutLazy: Array.from(document.images).filter(
          (img) => !img.loading
        ).length,
    ...
    throw new Error(
      `Failed to load page for detailed analysis: ${error.message}`
    );
  } finally {
    await browser.close();
  }
...
exports.breakdownResources = (resources) => {
  ...
  resources.forEach((r) => {
    ...
    else if (r.url.match(/\.(png|jpg|jpeg|gif|webp)(\?|$)/))
      totals.images += r.size;
    else if (r.url.match(/\.(woff|woff2|ttf|otf|eot)(\?|$)/))
      totals.fonts += r.size;
